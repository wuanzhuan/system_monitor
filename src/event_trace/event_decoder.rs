...
pub fn properties(
    ...
    while property_index < properties_array_end {
        ...
        let property_name = if !property_name.is_empty() { property_name.to_string().unwrap_or_default() } else { format!("no name:{property_index}") };
            ...
            && unsafe { property_info.Anonymous2.count } == 1
        {
            ...
            if in_type == TDH_INTYPE_INT8.0 || in_type == TDH_INTYPE_UINT8.0 {
                if user_data.len() - property_index as usize >= 1 {
                    int_values[property_index as usize] = u8::from_ne_bytes(user_data[user_data_index as usize ..user_data_index as usize + 1].try_into().unwrap()) as u16;
                }
            } else if in_type == TDH_INTYPE_INT16.0 || in_type == TDH_INTYPE_UINT16.0 {
                if user_data.len() - property_index as usize >= 2 {
                    int_values[property_index as usize] = u16::from_ne_bytes(user_data[user_data_index as usize ..user_data_index as usize + 2].try_into().unwrap());
                }
                ...
                || in_type == TDH_INTYPE_HEXINT32.0
            {
                if user_data.len() - property_index as usize >= 4 {
                    let v = u32::from_ne_bytes(user_data[user_data_index as usize ..user_data_index as usize + 4].try_into().unwrap());
                    int_values[property_index as usize] = if v > 0xffff { 0xffff } else { v as u16 };
                }
            }
        }
        ...
        let (array_count, is_array) = if (property_info.Flags.0 & PropertyParamCount.0) != 0 {
            ...
            if count_property_index >= property_index as u16 {
                error!("invalid count_property_index: {} index: {}", count_property_index, property_index);
        ...
        if is_struct {
            ...
            while array_index != array_count && (user_data_index as usize) < user_data.len() {
                ...
                let s = U16CStr::from_slice(prop_buffer.as_slice()).unwrap_or_default().to_string().unwrap_or_default();
            ...
            if is_array {
                ...
                properties_object.insert(property_name, PropertiesDecoded::String(properties_array[0].clone()));
            }
    ...
    *user_data_used = user_data_index;
